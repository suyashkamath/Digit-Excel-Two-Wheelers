...
#     """
#     Auto-detect which TW pattern this sheet follows.
#     FIXED: Better detection for regular TW vs 1+5 vs SAOD patterns.
#     """
#     # Check first 10 rows for clues - FIXED TYPE CONVERSION
#     sample_rows = []
#     for i in range(min(10, len(df))):
...
#     # Check for multi-year CD2 columns (SAOD pattern)
#     if "1 YEAR CD2" in sample_text and "2 YEAR CD2" in sample_text:
#         return "tw_saod_multiyear_cd2"
    
#     # Check for explicit 1+5 indicators
#     if any(x in sample_text for x in ["1+5", "1PLUS5", "1 + 5"]):
#         if "MAKE" in sample_text:
#             return "tw_1plus5_with_make"
#         else:
#             return "tw_1plus5_simple"
    
#     # Check for SAOD specific patterns (year-based CD2)
#     if "YEAR CD2" in sample_text or "YR CD2" in sample_text:
#         return "tw_saod_flexible_cd2"
    
#     # Check for Make column (might be 1+5 with Make)
#     if "MAKE" in sample_text and "CD2" in sample_text:
#         # But only if it doesn't have varied segments
#         if "MC <=" in sample_text or "MC_180" in sample_text or "SC/EV" in sample_text:
#             # Has multiple segment types - use regular TW processor
#             return "tw_simple"
#         else:
#             return "tw_1plus5_with_make"
    
#     # Check for multiple segment types (regular TW pattern)
#     segment_indicators = ["MC <=", "MC_180", "MC>350", "SC/EV", "HERO/HONDA", "OTHERS"]
#     if any(indicator in sample_text for indicator in segment_indicators):
#         return "tw_simple"
...
# # def process_tw_sheet(df, override_enabled, override_lob, override_segment, override_policy_type):
# #     """Fallback TW processor - handles multiple CD2 columns"""
# #     records = []
...
# #     for i in range(min(10, len(df))):
# #         row_str = " ".join([str(val).upper() if pd.notna(val) else "" for val in df.iloc[i]])
# #         if "AGENCY/PB" in row_str or "CLUSTER" in row_str:
# #             header_row = i
# #             break
...
# #     # Find all CD2 columns dynamically
# #     cd2_columns = []
# #     for idx, col in enumerate(df.columns):
# #         col_upper = str(col).upper()
# #         if "CD2" in col_upper or "SATP" in col_upper or "1+1" in col_upper:
# #             cd2_columns.append((idx, col))
    
# #     # If no CD2 columns found, use default indices
# #     if not cd2_columns:
# #         cd2_columns = [(3, "CD2_1"), (4, "CD2_2")]
# #         if len(df.columns) > 5:
# #             cd2_columns.append((5, "CD2_3"))
...
# #         segmentation = str(row[segment_col]).strip() if segment_col and pd.notna(row[segment_col]) else ""
# #         state = extract_state(cluster)
        
# #         # Determine segment and policy type from original segment
# #         segment_final = override_segment if override_enabled and override_segment else segmentation
        
# #         # Process each CD2 column
# #         for col_idx, col_name in cd2_columns:
# #             if col_idx >= len(df.columns):
# #                 continue
...
# #             # Determine policy type from column header
# #             col_upper = str(col_name).upper()
# #             if "SATP" in col_upper or "TP" in col_upper:
# #                 policy_type = "TP"
# #             elif "COMP" in col_upper or "OD" in col_upper:
# #                 policy_type = "Comp"
# #             else:
# #                 # Infer from segment name
# #                 policy_type = "TP" if "TP" in segmentation.upper() else "Comp"
            
# #             if override_enabled and override_policy_type:
...
# #     return records
# # def process_tw_sheet(df, override_enabled, override_lob, override_segment, override_policy_type):
# #     """Fallback TW processor - handles multiple CD2 columns with correct policy type detection"""
# #     records = []
    
# #     # Find header row with main headers (1+1, SATP)
# #     main_header_row = None
# #     sub_header_row = None
    
# #     for i in range(min(10, len(df))):
# #         row_str = " ".join([str(val).upper() if pd.notna(val) else "" for val in df.iloc[i]])
# #         if "1+1" in row_str or "SATP" in row_str:
# #             main_header_row = i
# #             sub_header_row = i + 1
# #             break
# #         elif "AGENCY/PB" in row_str or "CLUSTER" in row_str:
# #             sub_header_row = i
# #             break
...
# #     # Store main headers for policy type detection
# #     main_headers = {}
# #     if main_header_row is not None:
# #         for idx, val in enumerate(df.iloc[main_header_row]):
# #             if pd.notna(val):
# #                 main_headers[idx] = str(val).strip().upper()
    
# #     # Set columns from sub-header row
# #     df.columns = df.iloc[sub_header_row]
# #     df = df.iloc[sub_header_row + 1:].reset_index(drop=True)
# #     df.columns = [str(col).strip() if pd.notna(col) else f"Col_{i}" for i, col in enumerate(df.columns)]
    
# #     cluster_col = df.columns[0]
# #     segment_col = df.columns[1] if len(df.columns) > 1 else None
    
# #     # Find all CD2 columns dynamically with their policy types
# #     cd2_columns = []
# #     for idx, col in enumerate(df.columns):
# #         col_upper = str(col).upper()
# #         if "CD2" in col_upper or "CD1" in col_upper:
# #             # Determine policy type from main header
# #             policy_type = "Comp"  # Default
# #             status = "STP"  # Default
            
# #             # Check main header for this column
# #             if idx in main_headers:
# #                 main_header = main_headers[idx]
# #                 if "SATP" in main_header or "TP" in main_header:
# #                     policy_type = "TP"
# #                     status = "STP"
# #                 elif "1+1" in main_header or "COMP" in main_header or "OD" in main_header:
# #                     policy_type = "Comp"
# #                     status = "STP"
            
# #             cd2_columns.append((idx, col, policy_type, status, main_headers.get(idx, col)))
    
# #     # If no CD2 columns found, use default indices
# #     if not cd2_columns:
# #         # Column C (index 2) = 1+1 (Comp), Column E (index 4) = SATP (TP)
# #         cd2_columns = [
# #             (2, "CD1", "Comp", "STP", "1+1"),
# #             (3, "CD2", "Comp", "STP", "1+1"),
# #             (4, "CD2", "TP", "STP", "SATP")
...
# #     for idx, row in df.iterrows():
# #         cluster = str(row[cluster_col]).strip() if pd.notna(row[cluster_col]) else ""
# #         if not cluster or cluster.upper() in ["", "TOTAL", "GRAND TOTAL", "AGENCY/PB CLUSTERS", "AGENCY/PB SEG"]:
# #             continue
        
# #         segmentation = str(row[segment_col]).strip() if segment_col and pd.notna(row[segment_col]) else ""
# #         state = extract_state(cluster)
        
# #         # Determine segment from segmentation
# #         segment_final = override_segment if override_enabled and override_segment else segmentation
        
# #         # Process each CD2 column
# #         for col_idx, col_name, policy_type, status, main_header in cd2_columns:
# #             if col_idx >= len(df.columns):
# #                 continue
...
# #             # Override policy type if user specified
# #             final_policy_type = override_policy_type if (override_enabled and override_policy_type) else policy_type
            
# #             payout, formula, rule_exp = calculate_payout_with_formula(
# #                 lob_final, segment_final, final_policy_type, payin
# #             )
...
#     # Find all CD2 columns dynamically with their policy types (IGNORE CD1)
#     cd2_columns = []
#     for idx, col in enumerate(df.columns):
#         col_upper = str(col).upper()
#         # ONLY process CD2 columns, IGNORE CD1
#         if "CD2" in col_upper and "CD1" not in col_upper:
#             # Determine policy type from main header
#             policy_type = "Comp"  # Default
#             status = "STP"  # Default
...
#     # If no CD2 columns found, use default indices (SKIP CD1 at index 2)
#     if not cd2_columns:
#         # Column D (index 3) = 1+1/CD2 (Comp), Column E (index 4) = SATP/CD2 (TP)
#         cd2_columns = [
#             (3, "CD2", "Comp", "STP", "1+1"),
#             (4, "CD2", "TP", "STP", "SATP")
#         ]
...
#         # Process each CD2 column (CD1 is already filtered out)
#         for col_idx, col_name, policy_type, status, main_header in cd2_columns:
#             if col_idx >= len(df.columns):
#                 continue
...
# app = FastAPI(title="DIGIT Multi-LOB Processor API - Enhanced TW Edition")
...
# # ===============================================================================
# # ENHANCED TW SHEET PROCESSORS (ALL PATTERNS)
# # ===============================================================================

# def detect_tw_pattern(df):
#     """
#     Auto-detect which TW pattern this sheet follows:
#     - Pattern 1+5 with Make column (July, August, Sept, Oct)
#     - Pattern 1+5 with Remarks column (June Pattern 1)
#     - Pattern 1+5 simple (Jan, Feb, March, May)
#     - Pattern SAOD Multi-Year CD2 (June, July, Aug, Sept, Oct)
#     - Pattern SAOD Flexible CD2 (June Pattern 2)
#     - Pattern TP (various months)
#     """
#     # Check first 10 rows for clues
#     sample_text = " ".join([
#         " ".join(df.iloc[i].astype(str).str.upper().tolist()) 
#         for i in range(min(10, len(df)))
#     ])
...
#     for i in range(min(10, df.shape[0])):
#         row_values = df.iloc[i].astype(str).str.upper()
#         if "AGENCY/PB CLUSTERS" in row_values.values or "MAKE" in row_values.values:
#             header_row = i
#             break
...
#             "Location/Cluster": cluster,
#             "Make": make,
#             "Original Segment": row[segment_col] if segment_col else "",
#             "Mapped Segment": segment_final,
#             "LOB": lob_final,
#             "Policy Type": "TP",
#             "Status": "STP",
...
#             "State": state,
#             "Location/Cluster": cluster,
#             "Original Segment": row[segment_col] if segment_col and pd.notna(row[segment_col]) else "",
#             "Mapped Segment": segment_final,
#             "LOB": lob_final,
#             "Policy Type": "TP",
#             "Status": "STP",
...
# async def root():
#     return {
#         "message": "DIGIT Multi-LOB Processor API - Enhanced TW Edition",
#         "version": "3.0",
#         "features": [
#             "Auto-detects ALL TW patterns (1+5 and SAOD variations)",
#             "Handles monthly format changes automatically",
